...
/**
 * @brief Calculate the mean for this set
 * 
 * @param mu            This sets mean
 * @param sums          The sums for the elements in this set
 * @param counts        The counts for this sets elements
 * @param nFeatures     The number of features
 * @param rank          The process' rank
 */
void computeMu(float *mu, float *sums, int *counts, int nFeatures, int rank)
{
    int set = rank - 1; // the ranks set is itself (- 1 for the head node)
    int i_counts = 1;
    int offset = set * nFeatures; // rank offset for the sums 
    if (counts[set] != 0)
        i_counts = counts[set];
    for (int i = 0; i < nFeatures; i++)
        mu[i] = sums[offset + i] / i_counts;
}
...
int main(int argc, char *argv[])
{
    ...
    uswS = CLOCK();
    if (rank == 0)
        updateSetsWrapper(x, mu, sums, counts, sets, CLUSTERS, FEATURES, OBSERVATIONS, rank); // initialize sets based on initial means
    uswE = CLOCK() - uswS;

    MPI_Bcast(sums, CLUSTERS * FEATURES, MPI_FLOAT, 0, MPI_COMM_WORLD); // distribute data to all processes
    MPI_Bcast(counts, CLUSTERS, MPI_FLOAT, 0, MPI_COMM_WORLD); // distribute data to all processes

    if (rank > 0 && rank <= CLUSTERS) // update the means on different procs
    {
        computeMu(tempMu, sums, counts, FEATURES, rank); // update rank's set's mean
    }

    MPI_Allgather(tempMu, FEATURES, MPI_FLOAT, mu, FEATURES, MPI_FLOAT, MPI_COMM_WORLD); // have each node get all the means
